...
const actuacionSql = "WITH vDates AS (SELECT TO_DATE(:desde,'YYYY-MM-DD') + ROWNUM - 1 AS fecha FROM dual CONNECT BY rownum <= TO_DATE(:hasta,'YYYY-MM-DD') - TO_DATE(:desde,'YYYY-MM-DD') + 1)SELECT v.fecha, COUNT(p.idform) RES FROM vDates v LEFT JOIN (SELECT ff.idform,ff.fecfor FROM formularios ff INNER JOIN movimientosformulario mf ON mf.idform = ff.idform INNER JOIN movimientos mm ON mm.idmovi = mf.idmovi AND mm.tipmov = :tipmov WHERE ff.stafor = :stafor AND ff.reffor = :reffor) p ON TO_CHAR(p.fecfor) = TO_CHAR(v.fecha) GROUP BY v.fecha"
